...
test.describe('a spreadsheet', () => {
  ...
  test('guesses what to read and what to fill in, explains the split, and refuses a kept column in the prompt', async ({ page }) => {
    ...
    await expect(page.locator('textarea[data-field="prompt"]')).toHaveValue('{{text}}')
    await next(page)
    // The constant block (examples + format) is under Advanced debug on the answer-format screen.
    await page.click('text=Advanced debug')
  ...
  test('identifies a spreadsheet restored from a previous visit and can clear it', async ({ page }) => {
    ...
    await page.reload()
    await expect(page.locator('.data-preview .sheet-table tbody tr')).toHaveCount(5)
    // Two notices share the class (draft and sheet); the sheet one is the visible one here.
    await expect(page.locator('.sheet-section .restored-sheet-notice')).toContainText('restored from a previous visit')
    await page.click('.sheet-section .restored-sheet-notice button:has-text("Clear it")')
    await expect(page.locator('.sheet-section .restored-sheet-notice')).toBeHidden()
...
test.describe('the rest of the surface', () => {
  test('variations: blanks typed in the prompt become value lists on the same screen; every combination runs', async ({ page }) => {
    await page.click('button.chip[data-flow="sweep"]')
    await page.fill('textarea[data-field="sweep-prompt"]', 'Greet {{name}} from {{city}}')
    await expect(page.locator('.sweep-var')).toHaveCount(2)
    await expect(page.locator('button.wizard-next')).toBeDisabled()
    await expect(page.locator('.step.active .inline-blocker')).toContainText('{{name}}')
    await page.fill('.sweep-var[data-name="name"] textarea', 'Ann\nBob')
    await page.locator('.sweep-var[data-name="name"] textarea').press('Tab')
    await page.fill('.sweep-var[data-name="city"] textarea', 'Oslo\nRome\nLima')
    await page.locator('.sweep-var[data-name="city"] textarea').press('Tab')
    await expect(page.locator('.sweep-box')).toContainText('6 combinations')
    await expect(page.locator('.sweep-box')).toContainText('First of 6: Greet Ann from Oslo')
    await expect(page.locator('.sweep-box')).toContainText('Last: Greet Bob from Lima')
    // Removing a blank from the prompt keeps its values aside and drops it from the count.
    await page.fill('textarea[data-field="sweep-prompt"]', 'Greet {{name}}')
    await expect(page.locator('.sweep-var')).toHaveCount(1)
    await expect(page.locator('.sweep-box')).toContainText('2 combinations')
    await expect(page.locator('.sweep-box')).toContainText('Values kept for blanks no longer in the prompt: {{city}}')
    await page.fill('textarea[data-field="sweep-prompt"]', 'Greet {{name}} from {{city}}')
    await expect(page.locator('.sweep-box')).toContainText('6 combinations')
    await next(page, 3) // instructions (optional) -> format -> models
    await connectStub(page)
    await pickModel(page, 'stub-echo')
    await next(page, 2)
  ...
  test('pause holds new calls, resume continues, cancel stops; then the missing calls can be rerun', async ({ page }) => {
    ...
    await expect(page.locator('button.run-button')).toBeEnabled({ timeout: 15_000 })
    await page.click('button.run-button')
    await expect(page.locator('.run-tally')).toContainText('1 running', { timeout: 10_000 })
...
test.describe('refresh and scale', () => {
  test('a refresh keeps the sheet, the step, and the run on screen', async ({ page }) => {
    await page.click('button.chip[data-flow="sheet"]')
    await page.setInputFiles('input[type=file][accept*=".csv"]', { name: 'keep.csv', mimeType: 'text/csv', buffer: Buffer.from('text,label\nalpha beta gamma,x\nzeta eta theta,y\ndelta epsilon,\n') })
    await expect(page.locator('.data-preview .sheet-table tbody tr')).toHaveCount(3)
    await expect(page.locator('.partition-box')).toContainText('1 row will be filled in')
    await next(page)
    await page.fill('textarea[data-field="system"]', 'Label it.')
    await expect(page).toHaveURL(/#\/instructions$/)
    await page.reload()
    // Back on the instructions step, with the sheet still loaded and the template regenerated.
    await expect(page.locator('.step.active h2')).toHaveText('Instructions', { timeout: 15_000 })
    await expect(page.locator('textarea[data-field="prompt"]')).toHaveValue('{{text}}')
    await page.click('text=← Back')
    await expect(page.locator('.data-preview .sheet-table tbody tr')).toHaveCount(3)
    // Run, reload on the results screen: the run comes back with its rows.
    await next(page, 3)
    await connectStub(page)
    await pickModel(page, 'stub-echo')
    await next(page, 2)
    ...
    await page.reload()
    await expect(page.locator('.step.active h2')).toHaveText('Results', { timeout: 15_000 })
    await expect(page.locator('.final-table tbody tr')).toHaveCount(1)
    await expect(page.locator('.final-table')).toContainText('delta epsilon')
    await expect(page.locator('.status-line')).toContainText('Reopened')
  })

  test('above a thousand rows the table windows itself and still scrolls to the end', async ({ page }) => {
    test.setTimeout(180_000)
    await page.click('button.chip[data-flow="sweep"]')
    await page.fill('textarea[data-field="sweep-prompt"]', '{{a}} {{b}} {{c}}')
    for (const [name, values] of [['a', Array.from({ length: 10 }, (_, i) => `a${i}`)], ['b', Array.from({ length: 10 }, (_, i) => `b${i}`)], ['c', Array.from({ length: 11 }, (_, i) => `c${i}`)]] as const) {
      await page.fill(`.sweep-var[data-name="${name}"] textarea`, values.join('\n'))
      await page.locator(`.sweep-var[data-name="${name}"] textarea`).press('Tab')
    }
    await expect(page.locator('.sweep-box')).toContainText('1,100 combinations')
    await next(page, 3)
    await connectStub(page)
    await pickModel(page, 'stub-echo')
    await next(page)
    await page.fill('[data-field="concurrency"]', '32')
    await page.locator('[data-field="concurrency"]').press('Tab')
    await page.fill('[data-field="retries"]', '0')
    await page.locator('[data-field="retries"]').press('Tab')
    await next(page)
    await expect(page.locator('button.run-button')).toBeEnabled({ timeout: 30_000 })
    await page.click('button.run-button')
    await expect(page.locator('.run-tally')).toContainText('1100 of 1100 done', { timeout: 150_000 })
    const table = page.locator('.final-table.virtual')
    await expect(table).toHaveCount(1)
    const drawn = await table.locator('tbody tr.result-row').count()
    expect(drawn).toBeLessThan(200)
    await expect(table.locator('tbody tr.result-row').first()).toContainText('a0 · b0 · c0')
    await page.locator('.final-table-wrap').evaluate(el => { el.scrollTop = el.scrollHeight })
    await expect(table.locator('tbody tr.result-row').last()).toContainText('a9 · b9 · c10', { timeout: 10_000 })
    // The header stays in view at the bottom of the scroll.
    const header = table.locator('thead th').first()
    const wrapBox = await page.locator('.final-table-wrap').boundingBox()
    const headBox = await header.boundingBox()
    expect(headBox!.y).toBeGreaterThanOrEqual(wrapBox!.y - 1)
    expect(headBox!.y).toBeLessThan(wrapBox!.y + 60)
  })
})
